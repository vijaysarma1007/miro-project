...
export const Canvas = ({ boardId }: CanvasProps) => {
  ...
  const translateSelectedLayers = useMutation(
    ({ storage, self }, point: Point) => {
      ...
      // console.log("live layers", liveLayers);

      // console.log("self selection", self.presence.selection);
  ...
  const startMultiselection = useCallback(
    (current: Point, origin: Point) => {
      if (
        ...
        SELECTION_NET_THRESOLD
      ) {
        console.log("Attemptin gto selection net");
        setCanvasState({
          mode: CanvasMode.SelectionNet,
          origin,
          current,
  ...
  const onResizeHandlePointerDown = useCallback(
    (corner: Side, initialBounds: XYWH) => {
      // console.log({ corner: corner, initalBounds: initialBounds });
      history.pause();
      setCanvasState({
  ...
  return (
    <main className="h-dvh w-full relative bg-neutral-100 touch-none">
